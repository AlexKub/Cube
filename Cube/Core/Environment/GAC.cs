...
namespace Merlion.ECR.Update.Core.Environment
{
    ...
    //взято из https://www.codeproject.com/Articles/8285/GAC-API-Interface
    //похожие wrapper'ы (не разбирался) https://blogs.msdn.microsoft.com/junfeng/2004/09/14/sample-managed-gac-api-wrappers/
    ...
    /// </summary>
    enum IASSEMBLYCACHE_INSTALL_FLAG : uint
    {
    ...
    {
        //msdn: https://docs.microsoft.com/en-us/dotnet/framework/unmanaged-api/fusion/fusion-install-reference-structure
        /// <summary>
    ...
    interface IAssemblyCache
    {
        ...
        /// <returns></returns>
        [PreserveSig]
        int InstallAssembly( //msdn: https://docs.microsoft.com/en-us/dotnet/framework/unmanaged-api/fusion/iassemblycache-installassembly-method
            uint dwFlags,
    ...
    /// <summary>
    /// Управление Глобальным кешем сборок (GAC)
    /// </summary>
    public class GAC : IDisposable
    {
        //IAssemblyCache _gac = null;
        //IAssemblyEnum _enum = null;
        List<AssemblyName> _namesCollection = new List<AssemblyName>();

        readonly static ILoger m_loger = LogManager.GetDefaultLogSet<GAC>();

        /// <summary>
        /// Сборки, загруженные из GAC'a
        /// </summary>
        ...
        private GAC()
        {
            _namesCollection = new List<AssemblyName>();
        }
        ...
        public void ReLoad()
        {
            ...
            _namesCollection.AddRange(LoadAssembliesNames());
        }
        /// <summary>
        /// Проверка наличия библиотеки в GAC
        /// </summary>
        /// <param name="fInfo">Проверка наличия библиотеки в GAC</param>
        /// <param name="vers">Версия сборки. Не указано - проверяется соответствие только по имени</param>
        /// <returns>Возвращает флаг наличия в GAC сборки с соответствующим именем и версией (если указана)</returns>
        public bool Contains(FileInfo fInfo, Version vers = null)
        {
            string asmDisplayName = Path.GetFileNameWithoutExtension(fInfo.Name);

            if (vers == null)
            {
                if (Core_2_0.Utilites.Is_CLR_Assembly(fInfo.FullName))
                    vers = AssemblyName.GetAssemblyName(fInfo.FullName).Version;
            }

            return Contains(asmDisplayName, vers);
        }
        /// <summary>
        /// Проверка наличия библиотеки в GAC
        /// </summary>
        /// <param name="asmDisplayName">Имя библиотеки (без расширения)</param>
        /// <param name="vers">Версия сборки. Не указано - проверяется соответствие только по имени</param>
        /// <returns>Возвращает флаг наличия в GAC сборки с соответствующим именем и версией (если указана)</returns>
        public bool Contains(string asmDisplayName, Version vers)
        {
            try
            {
                return vers == null
                    ? _namesCollection.Any(ga => ga.Name.Equals(asmDisplayName))
                    : _namesCollection.Any(ga => ga.Name.Equals(asmDisplayName) && ga.Version.Equals(vers));
            }
            catch
            {
                return false;
        ...
        /// <summary>
        /// Загрузка информации о GAC
        /// </summary>
        /// <returns>Возвращает обёртку над GAC? готовую к использованию</returns>
        public static GAC Load()
        {
            try
            {
                GAC gac = new GAC();

                gac._namesCollection = new List<AssemblyName>(LoadAssembliesNames());

                return gac;
            }
            catch (Exception ex)
            {
                m_loger.Log("Возникло исключение при загрузке GAC'а", ex);
                return null;
            }
        }

        /// <summary>
        /// Установка библиотеки в GAC
        /// </summary>
        /// <param name="filePathName">Полное имя файла регистрируемой *.dll</param>
        /// <param name="force">Флаг принудительной установки. true - библиотека будет установлена, не смотря на версию установленной ранее в GAC</param>
        /// <returns>Возвращает флаг результата операции</returns>
        public static RegistrationResult InstallAssembly(string filePathName, bool force = false)
        {
            //взято отсюда:
            //https://stackoverflow.com/questions/2611108/register-a-dll-into-the-gac-but-then-it-doesnt-show-up-in-the-assembly-window/2611435
            //с некоторыми дополнениями (основной код AssemblyCache взят из другого места (см. header класса))

            IAssemblyCache iac = null;
            var result = RegistrationResult.UnKnown;

            try
            {
                iac = AssemblyCache.CreateAssemblyCache();

                //не удалось заставить работать с FUSION_INSTALL_REFERENCE
                //просто поставил null, как некоторые предлагают
                //
                //var refData = new FUSION_INSTALL_REFERENCE[1];
                //refData[0].guidScheme = AssemblyCache.FUSION_REFCOUNT_FILEPATH_GUID;
                //
                //using (var process = System.Diagnostics.Process.GetCurrentProcess())
                //    refData[0].szIdentifier = process.ExePathName();
                //
                //refData[0].cbSize = Marshal.SizeOf(refData);

                int hr = iac.InstallAssembly(force 
                        ? (uint)IASSEMBLYCACHE_INSTALL_FLAG.IASSEMBLYCACHE_INSTALL_FLAG_FORCE_REFRESH 
                        : (uint)IASSEMBLYCACHE_INSTALL_FLAG.IASSEMBLYCACHE_INSTALL_FLAG_REFRESH
                    , filePathName
                    , null);

                result = hr == VSConstants.S_OK ? RegistrationResult.Installed : RegistrationResult.Fail;
        ...
        /// </summary>
        /// <param name="asmName">Файл сборки</param>
        /// <param name="force">Флаг удаления сборки не смотря ни на что</param>
        /// <returns>Возвращает результат операции</returns>
        public static RegistrationResult UnInstallAssembly(FileInfo fInfo, bool force = true)
        {
            var nameWithoutExtension = Path.GetFileNameWithoutExtension(fInfo.Name);

            return UnInstallAssembly(nameWithoutExtension);
        }
        /// <summary>
        /// Удаление сборки из GAC
        /// </summary>
        /// <param name="asmName">Имя сборки (БЕЗ расширения *.dll)</param>
        /// <param name="force">Флаг удаления сборки не смотря ни на что (по умолчанию, чтобы наверняка)</param>
        /// <returns>Возвращает результат операции</returns>
        public static RegistrationResult UnInstallAssembly(string asmName, bool force = true)
        {
            IAssemblyCache iac = null;
            ...
            try
            {
                ...
                //просто поставил null, как некоторые предлагают
                //
                //string exePathName = string.Empty;
                //using (var process = System.Diagnostics.Process.GetCurrentProcess())
                //    exePathName = process.ExePathName();
                //
                //var refData = new FUSION_INSTALL_REFERENCE(AssemblyCache.FUSION_REFCOUNT_FILEPATH_GUID, exePathName, string.Empty);
                //refData.guidScheme = AssemblyCache.FUSION_REFCOUNT_FILEPATH_GUID;
                //
                //refData.cbSize = Marshal.SizeOf(refData);

                uint whatHappened;
                int hr = iac.UninstallAssembly(force ? (uint)IASSEMBLYCACHE_INSTALL_FLAG.IASSEMBLYCACHE_INSTALL_FLAG_FORCE_REFRESH : (uint)IASSEMBLYCACHE_INSTALL_FLAG.IASSEMBLYCACHE_INSTALL_FLAG_REFRESH
                    , asmName
                    , null
                    , out whatHappened);
                ...
                try
                {
                    result = (RegistrationResult)whatHappened;
                }
                catch
                {
        ...
        /// <summary>
        /// Перерегистрация сборки в GAC
        /// </summary>
        /// <param name="fInfo">Файл сборки</param>
        /// <param name="force">Флаг принудительных операций с GAC (установлен по умолчанию иначе могут быть проблемы с регистрацией из-за различия версий)</param>
        /// <returns>Возвращает результат операции</returns>
        public static RegistrationResult ReInstallAssembly(FileInfo fInfo, bool force = true)
        {
            string filePathName = fInfo.FullName;
            string asmName = Path.GetFileNameWithoutExtension(filePathName);

            return ReInstallAssembly(filePathName, asmName, force);
        }
        /// <summary>
        /// Перерегистрация сборки в GAC
        /// </summary>
        /// <param name="filePathName">Полный путь с именем к файлу сборки</param>
        /// <param name="asmName">Имя сборки (если не совпадает с именем файла)</param>
        /// <param name="force">Флаг принудительных операций с GAC (установлен по умолчанию иначе могут быть проблемы с регистрацией из-за различия версий)</param>
        /// <returns>Возвращает результат операции</returns>
        public static RegistrationResult ReInstallAssembly(string filePathName, string asmName = null, bool force = true)
        {
            if (string.IsNullOrWhiteSpace(asmName))
                asmName = Path.GetFileNameWithoutExtension(filePathName);

            var result = UnInstallAssembly(asmName, force);

            result = InstallAssembly(filePathName, force);
        ...
        /// <param name="vers">Версия сборки. Не указано - проверяется соответствие только по имени</param>
        /// <returns>Возвращает флаг наличия в GAC сборки с соответствующим именем и версией (если указана)</returns>
        public static bool HasAssembly(string asmDisplayName, Version vers = null)
        {
            try
            {
                using (var gac = GAC.Load())
                {
                    return gac.Contains(asmDisplayName, vers);
                }
            }
            catch (Exception ex)
            {
                m_loger.Log("Возникло исключение при проверке наличия сборки в GAC. Возвращён отрицательный результат.", ex
                    , new LogParameter("Искомое имя сборки", LogMessageBuilder.GetStringLogVal(asmDisplayName)));

                return false;
            }
        }

        private static IEnumerable<AssemblyName> LoadAssembliesNames()
        {
            try
            {
                var _list = new List<AssemblyName>();
                var en = AssemblyCache.CreateGACEnum();

                IAssemblyName comName = null;
                while (AssemblyCache.GetNextAssembly(en, out comName) == 0)
                {
                    if (comName != null)
                    {
                        _list.Add(GetAssemblyName(comName));
            ...
            {
                m_loger.Log("Возникло исключение при считываении данных библиотек из GAC", ex);
        ...
        static void ReleaseHRef(object o)
        {
            if (o != null)
            {
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                if (disposing)
                {
                    if (_namesCollection != null)
                    {
                        _namesCollection.Clear();
                        _namesCollection = null;
                    }
                }
