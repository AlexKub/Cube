...
{
    /// <summary>
    /// Логика работы с подписью файлов
    /// </summary>
    public static class SignManager
    {
        //скопировано из https://stackoverflow.com/questions/26344271/how-to-programmatically-code-sign-an-executable-with-a-pfx-bouncy-castle-or-oth/26372061#26372061
        ...
        /// <summary>
        /// ПОдпись файлов обычным сертификатом
        /// </summary>
        /// <param name="fInfo">Данные файла</param>
        public static void SignFile(FileInfo fInfo)
        {
            
            SignWithThumbprint(fInfo.FullName, "20bace54963f76229bfa174a59967afa9e328b11", string.Empty);
        }
        ...
        public static void SignWithThumbprint(string appPath, string thumbprint, string timestampUrl)
        {
            ...
            catch (CryptographicException ce)
            {
                ...
                StaticMethods.LogAndShow("Возникло исключение при загрузке сертификата подписи", ce);
            }
            catch (Exception e)
            {
                // do anything with this useful information?
                string exception = e.Message;
                StaticMethods.LogAndShow("Возникло исключение при подписывании файла", e
                    , new Merlion.ECR.Update.Core.Log.LogParameter("Путь к файлу", appPath));
            }
            finally
            {
                if (pSignerCert != IntPtr.Zero)
